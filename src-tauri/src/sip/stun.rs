...
pub async fn external_by_stun(
    ...
    info!(server = %stun_server, "Getting external IP by STUN server");
    ...
    let target = addrs
        .next()
        .ok_or_else(|| Error::Error("STUN server address not found".to_string()))?;
    ...
    let (len, _) = select! {
        _ = sleep(expires) => {
            info!(server = %stun_server, "STUN timeout");
    ...
    info!(external_ip = %socket, "External IP discovered");
...
async fn test_external_with_stun() -> Result<()> {
    ...
    for addr in addrs {
        info!(server = %addr, "STUN server");
    }
    let mut peer_bob = UdpConnection::create_connection("0.0.0.0:0".parse()?, None, None).await?;
    let expires = Duration::from_secs(5);
    external_by_stun(&mut peer_bob, "restsend.com:3478", expires).await?;
    info!(external_ip = ?peer_bob.get_addr(), "External IP discovered");
    Ok(())
}
